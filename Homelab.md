...
S         VPN ("Just in case everything else fails": vpn.domain)
H       Valkyrie S2 (More public facing stuff)
S         Traefik (Reverse proxy: *.domain)
...
S         shields.io (Surprise, it's open source, might as well host it!: shields.domain)
S         Code-Server (self-hosted github codespaces: code.domain)
